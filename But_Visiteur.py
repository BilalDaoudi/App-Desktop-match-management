...
class But_Visiteur(Tk):
    def __init__(self,match):
        super().__init__()
        self.match=match
        self.title(match.EquipeVisiteur.Nom)
    ...
    def Inisialise(self):
        ...
        self.btnGool=Button(self,text="GOOOOL",command=self.Ajouter_But_V)
        self.btnGool.place(x=100,y=90)

    def Ajouter_But_V(self):
        ...
        try:
            ...
            if num in self.CmpNumJ["values"]:
                if minu >= 1 and minu <= 120:
                    ...
                    But.Numero = num
                    But.Minute = minu
                    self.match.ADD_But_Visiteur(But)
                    messagebox.showinfo("But Visiteur", "But est Ajouté")
                else:
