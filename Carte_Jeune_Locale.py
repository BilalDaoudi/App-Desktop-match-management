...
class Carte_Jeune_Locale(Tk):
    def __init__(self,match):
        super().__init__()
        self.match=match
        self.title("Carte Jeune de "+match.EquipeLocale.Nom)
    ...
    def Inisialise(self):
        ...
        self.btnGool=Button(self,text="Carte Jeune",command=self.Ajouter_Carte_Jeune_L)
        self.btnGool.place(x=100,y=90)

    def Ajouter_Carte_Jeune_L(self):
        ...
        try:
            ...
            if num in self.CmpNumJ["values"]:
                if minu >= 1 and minu <= 120:
                    ...
                    carte.Numero = num
                    carte.Minute = minu
                    self.match.ADD_Carte_Jeune_Local(carte)
                    messagebox.showinfo("Carte Jeune Locale", "Carte Jeune est Ajouté")
                else:
