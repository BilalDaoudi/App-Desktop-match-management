...
class Carte_Rouge_Visiteur(Tk):
    def __init__(self,match):
        super().__init__()
        self.match=match
        self.title("Carte Rouge de "+match.EquipeVisiteur.Nom)
    ...
    def Inisialise(self):
        ...
        self.btnGool=Button(self,text="Carte Rouge",command=self.Ajouter_Carte_Rouge_V)
        self.btnGool.place(x=100,y=90)

    def Ajouter_Carte_Rouge_V(self):
        ...
        try:
            ...
            if num in self.CmpNumJ["values"]:
                if minu >= 1 and minu <= 120:
                    ...
                    carte.Numero = num
                    carte.Minute = minu
                    self.match.ADD_Carte_Rouge_Visiteur(carte)
                    messagebox.showinfo("Carte Rouge Visiteur", "Carte Rouge est Ajouté")
                else:
