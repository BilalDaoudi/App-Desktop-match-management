...
class Add_Joueur(Tk):
    ...
    def Initialise(self):
        ...
        self.lbl5 = Label(self, text="Numéro  :").place(x=20, y=140)
        self.lbl6 = Label(self, text="Poste  :").place(x=20, y=170)
        self.lbl7 = Label(self, text="Nationalité  :").place(x=20, y=200)
        self.lbl8 = Label(self, text="Equipe  :").place(x=20, y=230)

        self.entCin=Entry(self,textvariable=self.VarCin)
        self.entCin.place(x=130,y=20)
        self.entNom = Entry(self, textvariable=self.VarNom)
        self.entNom.place(x=130, y=50)
        self.entPrenom = Entry(self, textvariable=self.VarPrenom)
        self.entPrenom.place(x=130, y=80)
        self.entDtNaiss = Entry(self, textvariable=self.VarDtNaiss)
        self.entDtNaiss.place(x=130, y=110)
        self.entNum = Entry(self, textvariable=self.VarNum)
        self.entNum.place(x=130, y=140)
        self.entNation = Entry(self, textvariable=self.VarNatio)
        self.entNation.place(x=130, y=200)

        self.CmpPoste = Combobox(self)
        self.CmpPoste.place(x=130, y=170)

        self.CmpNomEqui = Combobox(self)
        self.CmpNomEqui.place(x=130, y=230)

        self.bntSupprimer = Button(self, text="Ajouter Joueur",command=self.Ajouter_Joueur)
        ...
        if (cin!="" and nom!="" and prenom!="" and dtNaissance!="" and numero!="" and poste in self.CmpPoste["values"] and nationalite!="" and nomequipe in self.ListNom):
            try:
                ...
                if numero <= 99 and numero >= 1:
                    ...
                    existe = equipe.Find_Joueur_Prin(cin)
                    if existe == -1:
                        equipe.ListJoueurPrin.append(J)
                        messagebox.showinfo("Ajoute Joueur", "Joueur est Ajouté avec succées")
                    else:
                        messagebox.showerror("Ajoute Joueur", "Joueur n'est pas Ajouté car déja existe")
                else:
            ...
            messagebox.showerror("Mauvaise Saisir", "Donner Tous Les Informations de Joueur")
